...
struct basic_explicit_inner_exception : basic_exception<CharT> {

    explicit basic_explicit_inner_exception(std::exception_ptr const& captured_exception,  const Allocator& allocator = Allocator());
    ...
    explicit basic_explicit_inner_exception(
        const std::exception_ptr& captured_exception,
        const std::source_location& source_location_value,
        const OtherAllocator& allocator = OtherAllocator());

    [[nodiscard]] std::expected<void, std::exception>  to_object(ndof::basic_object<CharT>& obj) const override;
...
struct basic_condition_check_exception : basic_explicit_inner_exception<ExceptionType, CharT, Allocator> {
    ...
    //       This would allow for more flexibility in passing string-like objects without requiring a specific string type.
