...
namespace ndof {
    ...
    struct precondition_check_error : std::logic_error {
        explicit precondition_check_error(std::size_t index)
            : std::logic_error("precondition check failed"), index_(index) {}
    ...
    struct postcondition_check_error : std::logic_error {
        explicit postcondition_check_error(std::size_t index)
            : std::logic_error("postcondition check failed"), index_(index) {}
    ...
    template <typename... Args> struct check<bool(Args...)> {
        ...
        check(Callable&& callable) : callable_(std::forward<Callable>(callable)) {}
    ...
    template <typename Callable>
    check(Callable&&) -> check<detail::callable_signature_t<Callable>>;

    template <typename Result, typename... Args>
    check(Result (*)(Args...)) -> check<bool(Args...)>;
    ...
    template <typename Result, std::size_t N> struct postcondition_check_builder {
        ...
        template <typename Body>
            requires std::invocable<Body&> &&
                     std::convertible_to<std::invoke_result_t<Body&>, Result>
        [[nodiscard]] Result body(Body&& body_callable) const {
            ...
            for (const auto& condition : checks_) {
                if (!condition(result)) {
                    throw postcondition_check_error(index);
                }
                ++index;
            }
            return result;
    ...
    precondition_check(std::initializer_list<check<bool()>> checks) {
        ...
        for (const auto& condition : checks) {
            if (!condition()) {
                throw precondition_check_error(index);
            }
            ++index;
        }
